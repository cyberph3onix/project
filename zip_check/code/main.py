...
if __name__ == "__main__":
    ...
    def _pick_field(row: dict, candidates: List[str]) -> Optional[str]:
    ...
    def process_test_csv(test_csv_path: str, books_dir: str, output_csv_path: str) -> None:
        if not os.path.exists(test_csv_path):
            raise FileNotFoundError(f"test.csv not found at {test_csv_path}")

        # Use latin-1 to handle encoding issues
        with open(test_csv_path, newline='', encoding='latin-1') as f:
            reader = csv.DictReader(f)
            rows = list(reader)

        with open(output_csv_path, "w", newline='', encoding='utf-8') as out_f:
            writer = csv.writer(out_f)
            writer.writerow(["story_id", "prediction"])  # header

            for row in rows:
                # heuristically find columns
                story_key = _pick_field(row, ["story_id", "id", "storyid"])
                book_key = _pick_field(row, ["book_name", "book", "bookname", "novel", "novel_name"])
                backstory_key = _pick_field(row, ["hypothetical_backstory", "backstory", "hypothesis", "hypothetical", "content", "caption"])

                story_id = row.get(story_key) if story_key else None
                book_name = row.get(book_key) if book_key else None
                backstory_text = row.get(backstory_key) if backstory_key else ""

                if not story_id:
                    # skip rows without story identifier
                    continue

                novel_text = ""
                if book_name:
                    candidate = os.path.join(books_dir, f"{book_name}.txt")
                    if os.path.exists(candidate):
                        try:
                            with open(candidate, encoding='utf-8', errors='replace') as bf:
                                novel_text = bf.read()
                        except Exception:
                            pass
                ...
                try:
                    final_label, _results = verify_backstory(novel_text, backstory_text)
                except Exception:
                    final_label = 0

                writer.writerow([story_id, int(final_label)])

    if os.path.exists(test_csv):
        print(f"Processing test CSV: {test_csv}")
        process_test_csv(test_csv, books_dir, output_csv)
        print(f"Wrote story-level results to: {output_csv}")
    else:
        print("=" * 70)
        ...
        # Save report (kept for local debugging; dataset mode won't write JSON)
        save_report(results)
